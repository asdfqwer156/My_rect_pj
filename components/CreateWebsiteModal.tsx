...
const CreateWebsiteModal: React.FC<{
    ...
    const handleGenerateInfo = async () => {
        ...
        setError(null);
        try {
            // FIX: Using Gemini to generate title and description based on URL
            const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });
            const model = 'gemini-2.5-flash';
            const prompt = `Based on the URL "${url}", please provide a suitable title and a brief description for this website. The description should be a single sentence, no more than 100 characters.`;
            ...
            console.error(e);
            setError("Failed to generate website information. Please fill it in manually.");
        } finally {
            setIsGenerating(false);
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onAdd({
            ...
            // A placeholder thumbnail, this could be improved with a screenshot service
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <div className="flex gap-2 mt-1">
                            <input
                                ...
                                className="block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm text-gray-900 dark:text-white"
                            ...
                            <button
                                ...
                                className="px-4 py-2 text-sm font-medium text-white bg-primary-600 border border-transparent rounded-md shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:bg-gray-400 disabled:cursor-not-allowed"
                            >
                                {isGenerating ? '생성 중...' : '정보 생성'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        ...
                        <button
                            ...
                            className="px-4 py-2 text-sm font-medium text-white bg-primary-600 border border-transparent rounded-md shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:bg-gray-400"
                        >
                            추가하기
                        </button>
                    </div>
